...
async function downloadZip() {
  ...
  for (const [i, it] of currentItems.entries()) {
    try {
      ...
      zip.file(
        `photo_${String(i + 1).padStart(2, "0")}_${currentSize}.${ext}`,
        blob
      );
      done++;
...
apiKeyEl.addEventListener("input", () => {
  if (rememberKeyEl.checked) rememberKeyMaybe();
});
...
keywordEl.addEventListener("keydown", (e) => {
  if (e.key === "Enter") {
    e.preventDefault(); // stop form submission or other defaults
    renderSet();
  }
});
